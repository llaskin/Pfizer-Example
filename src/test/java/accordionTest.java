...
//        eyes.check("Full Screen popup", Target.window().fully());
        List<WebElement> accordion = driver.findElements(By.cssSelector(".accordion-row"));
        ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", accordion.get(0));


        for(i = 0; i <= accordion.size(); i++)
        {
            accordion.get(i).findElement(By.cssSelector(".accordion-header")).click();
            eyes.check("Accordion " + i, Target.window().fully());
